...
inferType' e = case e of
  ...
  LNIL τ -> Just (TVec τ)
  LCONS e es | Just τ' <- inferType' e
             , Just (TVec τ) <- inferType' es
             , τ' == τ
             -> Just (TVec τ)
             | otherwise -> Nothing
...
compileE m (LNIL _) = emptyCircuit m
compileE m (LCONS p ps) = append' (compileE m p) (compileE m ps)
