...
label :: Ord p => [DSL p] -> (Int, [LDSL p Int], [LDSL p Int])
label programs = (m, labeledBodies, labeledBindings) where
  (m, labeledBodies, labeledBindings, _) = labelAll programs

  {-@ labelAll :: [DSL p] ->
                  (m:Nat, [LDSL p Int], [LDSL p Int], Env p Int)
                             <\m     -> {l:[LDSL p (Btwn 0 m)] | true},
                              \_ m   -> {l:[LDSL p (Btwn 0 m)] | true},
                              \_ _ m -> {v:Env   p (Btwn 0 m)  | true}> @-}
  labelAll :: Ord p => [DSL p] -> (Int, [LDSL p Int], [LDSL p Int], Env p Int)
  labelAll programs = foldl go (0, [], [], M.empty) programs where
    {-@ go :: (m:Nat, [LDSL p Int], [LDSL p Int], Env p Int)
                            <\m     -> {l:[LDSL p (Btwn 0 m)] | true},
                             \_ m   -> {l:[LDSL p (Btwn 0 m)] | true},
                             \_ _ m -> {v:Env   p (Btwn 0 m)  | true}>
           -> DSL p
           -> (m:Nat, [LDSL p Int], [LDSL p Int], Env p Int)
                            <\m     -> {l:[LDSL p (Btwn 0 m)] | true},
                             ...
                             \_ _ m -> {v:Env   p (Btwn 0 m)  | true}> @-}
    go :: Ord p =>
          (Int, [LDSL p Int], [LDSL p Int], Env p Int) -> DSL p ->
          (Int, [LDSL p Int], [LDSL p Int], Env p Int)
    go (nextIndex, acc1, acc2, env) program =
      let (nextIndex', labeledBody, labeledBindings, env') = label' program nextIndex env
      in (nextIndex',
          acc1 ++ labeledBody,
  ...
  {-@ label2 :: m0:Nat ->
                ...
                Env p (Btwn 0 m0) ->
                (m:{Int | m > m0}, LDSL p Int, LDSL p Int, [LDSL p Int],
                 Env p Int)<\m       -> {v:LDSL  p (Btwn 0 m)  | true},
                            \_ m     -> {v:LDSL  p (Btwn 0 m)  | true},
                            \_ _ m   -> {l:[LDSL p (Btwn 0 m)] | true},
                            \_ _ _ m -> {v:Env   p (Btwn 0 m)  | true}> @-}
  label2 :: Ord p => Int -> DSL p -> DSL p -> Env p Int ->
            (Int, LDSL p Int, LDSL p Int, [LDSL p Int], Env p Int)
  label2 nextIndex arg1 arg2 env =
    let i = nextIndex -- for the current gate
        (i1, [arg1'], bs1, env1) = label' arg1 (i+1) env
        (i2, [arg2'], bs2, env2) = label' arg2 i1 env1
    in (i2, arg1', arg2', bs1 ++ bs2, env2)
  ...
  label' p nextIndex env = case M.lookup p env of
    ...
    Nothing -> let i = nextIndex in case p of
      ...
      VAR s -> (i+1, [LWIRE i], [LVAR s i], add (p,i) env)
      CONST x -> (i+1, [LWIRE i], [LCONST x i], add (p,i) env)

      ADD p1 p2 -> (i', [LWIRE i], bs' ++ [LADD p1' p2' i], add (p,i) env')
        where (i', p1', p2', bs', env') = label2 i p1 p2 env
      SUB p1 p2 -> (i', [LWIRE i], bs' ++ [LSUB p1' p2' i], add (p,i) env')
        where (i', p1', p2', bs', env') = label2 i p1 p2 env
      MUL p1 p2 -> (i', [LWIRE i], bs' ++ [LMUL p1' p2' i], add (p,i) env')
        where (i', p1', p2', bs', env') = label2 i p1 p2 env
      DIV p1 p2 -> (i', [LWIRE i], bs' ++ [LDIV p1' p2' i], add (p,i) env')
        where (i', p1', p2', bs', env') = label2 i p1 p2 env

      NOT p1    -> (i', [LWIRE i], bs' ++ [LNOT p1' i], add (p,i) env')
        where (i', [p1'], bs', env') = label' p1 (i+1) env
      AND p1 p2 -> (i', [LWIRE i], bs' ++ [LAND p1' p2' i], add (p,i) env')
        where (i', p1', p2', bs', env') = label2 i p1 p2 env
      OR  p1 p2 -> (i', [LWIRE i], bs' ++ [LOR  p1' p2' i], add (p,i) env')
        where (i', p1', p2', bs', env') = label2 i p1 p2 env
      XOR p1 p2 -> (i', [LWIRE i], bs' ++ [LXOR p1' p2' i], add (p,i) env')
        where (i', p1', p2', bs', env') = label2 i p1 p2 env

      UnsafeNOT p1    -> (i', [LWIRE i], bs' ++ [LUnsafeNOT p1' i], add (p,i) env')
        where (i', [p1'], bs', env') = label' p1 (i+1) env
      UnsafeAND p1 p2 -> (i', [LWIRE i], bs' ++ [LUnsafeAND p1' p2' i], add (p,i) env')
        where (i', p1', p2', bs', env') = label2 i p1 p2 env
      UnsafeOR  p1 p2 -> (i', [LWIRE i], bs' ++ [LUnsafeOR  p1' p2' i], add (p,i) env')
        where (i', p1', p2', bs', env') = label2 i p1 p2 env
      UnsafeXOR p1 p2 -> (i', [LWIRE i], bs' ++ [LUnsafeXOR p1' p2' i], add (p,i) env')
        where (i', p1', p2', bs', env') = label2 i p1 p2 env

      EQL p1 p2 -> label' (ISZERO (p1 `SUB` p2)) nextIndex env
      ISZERO p1 -> let w = i+1
                       (i1, [p1'], bs, env') = label' p1 (w+1) env
                   in (i1, [LWIRE i], bs ++ [LISZERO p1' w i], add (p,i) env')
      EQLC p1 k -> let w = i+1
                       (i1, [p1'], bs, env') = label' p1 (w+1) env
                   in (i1, [LWIRE i], bs ++ [LEQLC p1' k w i], add (p,i) env')


      NIL -> (i, [], [], env)
      CONS h ts -> let (i', h', bs', env') = label' h i env
                       (i'', ts', bs'', env'') = label' ts i' env'
                       bs = bs' ++ bs''
                   in (i'', h' ++ ts', bs, env'') where
